...
TEST(StringUtil_join, randomSep)
{
	...
	while(numTests--)
	{
		...
		std::string manual;
		std::for_each(values.begin(), values.end(), [&](const std::string&  v)->void
				{
				manual += manual.empty() ? v : sep + v;
				});
...
{
	ASSERT_STREQ("abcd", StringUtil::getHead("abcd"+pac::delim+"efg").c_str());
}
...
{
	ASSERT_STREQ("abcd.ext", StringUtil::getTail(pac::delim+"abcd.ext").c_str());
}
TEST(StringUtil_getTail, absolute2Delim)
{
	ASSERT_STREQ("efg", StringUtil::getTail(pac::delim+"abcd"+pac::delim+"efg").c_str());
}
