...
void testOutputString(const std::string& s, size_t textWidth)
{
	...
	std::for_each(sv.begin(), sv.end(), [&](const std::string& val)->void
	{
		...
		for (StringVector::iterator iter = tmp.begin(); iter != tmp.end(); iter += 2) 
		{
	...
	for (size_t col = 0; col < numCol; ++col) 
	{
		...
		{
			EXPECT_GE(grid[row].size(), numCol - 1); //at lest numCol -1 is full filled
	...
	if(numCol > 1)
		EXPECT_LE(totalColWidth, textWidth);
}
...
{
public:
	PimplDefaultPattern(size_t textWidth):
		DefaultPattern(textWidth){}
...
protected:
	...
	TestConsolePattern80()
	{
		mPattern = new PimplDefaultPattern(80);
	}
	virtual ~TestConsolePattern80()
	{
		delete mPattern;
	}
...
TEST_F(TestConsolePattern80, getCandidateNumColumns) 
{
	...
	ASSERT_EQ(1, sv.size());
	ASSERT_EQ(1, sv[0]);
	sv = mPattern->getCandidateNumColumns(80,2,75);//min + max  + 2* space > 80
	ASSERT_EQ(1, sv.size());
	ASSERT_EQ(1, sv[0]);
	sv = mPattern->getCandidateNumColumns(80,8,69); //min + max + 2*space > 80
	ASSERT_EQ(1, sv.size());
	ASSERT_EQ(1, sv[0]);
	sv = mPattern->getCandidateNumColumns(12,8,20); //normal case, numItem > maxCol 
	ASSERT_EQ(5, sv.size());
	size_t s[] = {1,2,3,4,6};
	...
	sv = mPattern->getCandidateNumColumns(8,8,20); //normal case, numItem = maxCol 
	ASSERT_EQ(5, sv.size());
	size_t s1[] = {1,2,3,4,8};
	for (size_t i = 0; i < sv.size(); ++i) 
	{
	...
	sv = mPattern->getCandidateNumColumns(9,6,20); //normal case, numItem < maxCol 
	ASSERT_EQ(5, sv.size());
	size_t s2[] = {1,2,3,5,9};
	for (size_t i = 0; i < sv.size(); ++i) 
	{
...
{
	resetBuffer("1",1);
	const std::string&& s = mPattern->applyPattern(mBuffer.begin(), mBuffer.end());
