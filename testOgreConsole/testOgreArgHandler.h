...
TEST_F(TestOgreScene, handler_sceneNode) {
  std::unique_ptr<ArgHandler> handler(sgArgLib.createArgHandler("sceneNode"));
...
TEST_F(TestOgreScene, handler_childNode) {
  std::unique_ptr<ArgHandler> handler(sgArgLib.createArgHandler("childNode"));
  EXPECT_TRUE(handler->validate(mEntNode0Nameid));
...
TEST_F(TestOgreScene, handler_t_sceneNode) {
  std::unique_ptr<ArgHandler> handler(sgArgLib.createArgHandler("t_sceneNode"));
  //  sceneNode ("0")
  EXPECT_TRUE(handler->validate(mEntNode0Nameid));
  EXPECT_TRUE(handler->validate(mDynamicRootNodeNameid));
  EXPECT_TRUE(handler->validate(mStaticRootNodeNameid));
...
TEST_F(TestOgreScene, handler_t_movable) {
  std::unique_ptr<ArgHandler> handler(sgArgLib.createArgHandler("t_movable"));
  //  moType movable
  ...
  EXPECT_TRUE(handler->validate(mEntNode0Nameid + " Item " + mOgreHeadNameid));
...
TEST_F(TestOgreScene, handler_resource) {
  std::unique_ptr<ArgHandler> handler(sgArgLib.createArgHandler("mesh"));
...
TEST_F(TestOgreScene, handler_position) {
  std::unique_ptr<ArgHandler> handler(
      sgArgLib.createArgHandler("scene_position"));
...
TEST_F(TestOgreScene, handler_direction) {
  std::unique_ptr<ArgHandler> handler(
      sgArgLib.createArgHandler("scene_direction"));
  EXPECT_TRUE(handler->validate("1 2 3"));
  EXPECT_TRUE(
      handler->validate("dirOfNode " + mEntNode0Nameid + " TS_WORLD AXIS_Z"));
  Ogre::Vector3 v[3];
...
TEST_F(TestOgreScene, handler_position_transform) {
  std::unique_ptr<ArgHandler> handler(
      sgArgLib.createArgHandler("sceneNode_lookAt"));
...
TEST_F(TestOgreScene, handler_direction_transform) {
  std::unique_ptr<ArgHandler> handler(
      sgArgLib.createArgHandler("sceneNode_direction"));
  EXPECT_TRUE(handler->validate("1 2 3 TS_WORLD"));
  EXPECT_TRUE(handler->validate(
      "dirOfNode " + mEntNode0Nameid + " TS_WORLD AXIS_Z TS_WORLD "));
  Ogre::Vector3 v[3];
  mEntNode0->_getDerivedOrientationUpdated().ToAxes(v);
  EXPECT_TRUE(cmpRealStrings(
      Ogre::StringConverter::toString(v[2]), handler->getUniformValue()));
...
TEST_F(TestOgreScene, compWorkspace) {
  std::unique_ptr<ArgHandler> handler(
      sgArgLib.createArgHandler("compWorkspace")); 
  //because no frame will be updated, workspaces remain in queue
